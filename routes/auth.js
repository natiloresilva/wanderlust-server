...
router.post(
  "/signup",
  // revisamos si el user no estÃ¡ ya logueado usando la funciÃ³n helper (chequeamos si existe req.session.currentUser)
  // isNotLoggedIn(),
  // revisa que se hayan completado los valores de username y password usando la funciÃ³n helper
  validationLoggin(),
  async (req, res, next) => {
...
// POST '/logout'
// revisa si el usuario estÃ¡ logueado usando la funciÃ³n helper (chequea si la sesiÃ³n existe), y luego destruimos la sesiÃ³n
router.post("/logout", isLoggedIn(), (req, res, next) => {
  req.session.destroy();
  //  - setea el cÃ³digo de estado y envÃ­a de vuelta la respuesta
  res
...
// revisa si el usuario estÃ¡ logueado usando la funciÃ³n helper (chequea si existe la sesiÃ³n), y devuelve un mensaje
router.get("/private", isLoggedIn(), (req, res, next) => {
  //  - setea el cÃ³digo de estado y devuelve un mensaje de respuesta json
  res
    .status(200) // OK
...
// GET '/me'
// chequea si el usuario estÃ¡ logueado usando la funciÃ³n helper (chequea si existe la sesiÃ³n)
router.get("/me", isLoggedIn(), (req, res, next) => {
  // si estÃ¡ logueado, previene que el password sea enviado y devuelve un json con los datos del usuario (disponibles en req.session.currentUser)
